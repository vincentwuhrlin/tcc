...
* De plus, il va falloir à la fin s'assurer que tout ce que a présenté Siemens existe réellment. La aussi, il faut m'aider à imaginer et formaliser des questions que l'on pourrait poser à Siemens
...
Ok, j'ai testé un re-export.
Plusieurs points  :
* Je t'ai mis un exmeple de PROJECT_INSTRUCTIONS.md pour "noa-security-gateway". Et dedans on ne parle pas de Umbrella, du contexte de l'équipe. Donc oui le hub va me répondre de façon généraliste et m'aiguillé sur ce projet. Mais ensuite dans ce projet, le contexte ne sera plus le même non ?
* Même si c'est un peu hors scope, un des managers s'intéresse à la convergence IT hors europe, aux états unis, et donc d'autres standard que NOA. Comment traité ça, dans un nouveau chapitre à part, avec d'autres documents ? (et donc nouveau projet)

Je t'ai mis les instructions de tous les projets. Je trouve ça bizarre d'avoir en haut :
Tu es un expert en sécurité OT spécialisé dans le NOA Security Gateway

Puis dans le contexte projet
**Notre rôle :** nous ne sommes pas les architectes OT

Ca posera pas de probleme pour la reflexion Claude ?
